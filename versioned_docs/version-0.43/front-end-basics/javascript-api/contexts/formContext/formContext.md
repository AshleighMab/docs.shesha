...
`contexts.formContext` is available in any script on the form, alongside the other standard script variables. It is not one of the fixed top-level script variables like `data` or `form` - it is one entry inside the generic `contexts` object, which is resolved to whichever data contexts are available at that point in the form tree.
